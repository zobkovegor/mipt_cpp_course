...
decltype(auto) get(TuplePos<N, Head, Tail...>&& tuple) {
  return std::forward<Head>(tuple.TupleVal<N, Head>::value);
}

template <size_t N, typename Head, typename... Tail>
decltype(auto) get(const TuplePos<N, Head, Tail...>&& tuple) {
  return std::forward<const Head>(tuple.TupleVal<N, Head>::value);
}
...
decltype(auto) get(Tuple<Ts...>&& tuple) {
  static_assert(type_count<T, Ts...>::value == 1);
  constexpr size_t idx = type_index<T, Ts...>::value;
  return std::forward<T>(get<idx>(tuple));
}
...
decltype(auto) get(const Tuple<Ts...>&& tuple) {
  static_assert(type_count<T, Ts...>::value == 1);
  constexpr size_t idx = type_index<T, Ts...>::value;
  return std::forward<const T>(get<idx>(tuple));
}
...
template <typename... Ts>
struct TupleCatTypes<Tuple<Ts...>> {
  using type = Tuple<Ts...>;
};

template <typename... Ts, typename... Us, typename... Rest>
struct TupleCatTypes<Tuple<Ts...>, Tuple<Us...>, Rest...> {
  using type = typename TupleCatTypes<Tuple<Ts..., Us...>, Rest...>::type;
};

template <typename... Tuples>
using TupleCatTypes_t = typename TupleCatTypes<Tuples...>::type;
...
template <typename... Ts>
struct tuple_size_helper<Tuple<Ts...>> {
  static constexpr size_t value = TuplePos<0, Ts...>::tuple_size();
};

template <typename T>
constexpr size_t tuple_size_v = tuple_size_helper<std::remove_reference_t<T>>::value;

template <size_t N, typename TupleT>
decltype(auto) get_from_tuple(TupleT&& tuple) {
  return get<N>(std::forward<TupleT>(tuple));
}

template <size_t N, typename TupleT, typename... Rest>
decltype(auto) get_from_tuple(TupleT&& tuple, Rest&&... rest) {
  if constexpr (N < tuple_size_v<std::remove_reference_t<TupleT>>) {
    return get<N>(std::forward<TupleT>(tuple));
  } else {
    return get_from_tuple<N - tuple_size_v<std::remove_reference_t<TupleT>>>(std::forward<Rest>(rest)...);
  }
}

template <typename... Tuples>
TupleCatTypes_t<std::remove_reference_t<Tuples>...> tupleCat(Tuples&&... tuples) {
  using ResultType = TupleCatTypes_t<std::remove_reference_t<Tuples>...>;
  constexpr size_t total_size = (tuple_size_v<std::remove_reference_t<Tuples>> + ... + 0);

  auto construct = []<size_t... Is>(std::index_sequence<Is...>, auto&&... ts) {
    return ResultType(get_from_tuple<Is>(std::forward<decltype(ts)>(ts)...)...);
  };

  return construct(std::make_index_sequence<total_size>{}, std::forward<Tuples>(tuples)...);
}